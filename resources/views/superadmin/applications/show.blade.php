...
@section('content')
    <div class="max-w-[1100px] mx-auto space-y-4">
        ...
        <div
            ...
            <div class="flex items-center gap-2">
                ...
                @php
                    ...
                    $currentCfgSA = $statusConfigSA[$application->status] ?? ['label' => ucwords(str_replace('_', ' ', $application->status)), 'bg' => 'bg-gray-100', 'text' => 'text-gray-800', 'border' => 'border-gray-300', 'dot' => 'bg-gray-500', 'icon' => 'swap_horiz'];
                @endphp

                <div class="relative" x-data="{ open: false }">
                    <form id="stageFormSA" action="{{ route('superadmin.applications.update-status', $application) }}" method="POST">
                        @csrf @method('PATCH')
                        <input type="hidden" name="status" id="stageStatusInputSA" value="{{ $application->status }}">
                    </form>

                    {{-- Main Status Trigger Button --}}
                    <button type="button" @click="open = !open"
                        class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-bold border transition-all shadow-2xs hover:shadow-xs cursor-pointer select-none {{ $currentCfgSA['bg'] }} {{ $currentCfgSA['text'] }} {{ $currentCfgSA['border'] }}">
                        <span class="w-2 h-2 rounded-full {{ $currentCfgSA['dot'] }} shrink-0 {{ $application->status === 'interview_scheduled' ? 'animate-pulse' : '' }}"></span>
                        <span class="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Stage:</span>
                        <span>{{ $currentCfgSA['label'] }}</span>
                        ...
                        <div class="max-h-80 overflow-y-auto py-1 divide-y divide-gray-100">
                            ...
                            <div class="py-1">
                                @foreach(['submitted', 'new', 'under_review'] as $st)
                                    @php $cfg = $statusConfigSA[$st]; @endphp
                                    <button type="button" @click="open = false; selectStageSA('{{ $st }}')"
                ...
                @endif
                <button onclick="document.getElementById('emailComposerSA').classList.toggle('hidden')"
        ...
        <div id="emailComposerSA" class="hidden bg-white border border-gray-200 shadow-md rounded-2xl print:hidden">
            ...
            <div class="p-5">
                <form action="{{ route('superadmin.applications.send-email', $application) }}" method="POST" x-data="{
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <button type="button"
                                onclick="document.getElementById('emailComposerSA').classList.add('hidden')"
            ...
            <div class="p-6 border-b border-gray-200 bg-amber-50/20">
                <div class="flex items-center justify-between mb-4 pb-2 border-b border-gray-100">
                    <h3 class="text-xs font-extrabold uppercase tracking-widest text-[#21255E] flex items-center gap-2">
                        <span class="material-symbols-outlined text-[18px] text-[#21255E]">rate_review</span>
                ...
                <div class="print:hidden mb-5">
                    <form action="{{ route('superadmin.applications.update-notes', $application) }}" method="POST" class="space-y-3">
                        @csrf
                        @method('PATCH')
                        <div>
                            ...
                            <textarea name="admin_notes" rows="4"
                                placeholder="Enter interview feedback, panel evaluation remarks, strengths, key discussion points, or hiring recommendation..."
                ...
                @if($application->admin_notes)
                    <div class="hidden print:block border border-gray-200 rounded-xl p-4 bg-white text-xs text-gray-800 leading-relaxed font-sans mb-4">
                                    ...
                                    @foreach($application->interviews as $inv)
                                        <tr>
                                            <td class="px-3.5 py-2.5 font-bold text-gray-900 whitespace-nowrap">
                                                ...
                                                <span class="text-gray-500 font-normal block text-[11px]">{{ \Carbon\Carbon::parse($inv->scheduled_time)->format('h:i A') }}</span>
                                            </td>
                                            <td class="px-3.5 py-2.5 text-gray-700">
                                                <span class="font-semibold">{{ ucfirst(str_replace('_', ' ', $inv->location_type)) }}</span>
                                                @if($inv->location_address_or_link)
                                                    <span class="text-gray-500 block text-[11px] truncate max-w-xs">{{ $inv->location_address_or_link }}</span>
                                                @endif
                                            </td>
                                            <td class="px-3.5 py-2.5 text-gray-700">
                                                {{ $inv->panel_members ?: '—' }}
                                            ...
                                            <td class="px-3.5 py-2.5 whitespace-nowrap">
                                                <span class="inline-block px-2 py-0.5 rounded text-[10px] font-extrabold uppercase {{ $inv->status === 'completed' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : ($inv->status === 'cancelled' ? 'bg-rose-50 text-rose-700 border border-rose-200' : 'bg-amber-50 text-amber-700 border border-amber-200') }}">
                                                    {{ $inv->status }}
                                                </span>
                                                @if($inv->score !== null)
                                                    <span class="font-extrabold text-gray-900 block text-[11px] mt-0.5">{{ $inv->score }} / 100</span>
                                                @endif
                                            </td>
    ...
    @php
        $latestInterviewSA = $application->interviews ? $application->interviews->sortByDesc('id')->first() : null;
    @endphp

    {{-- ── INTERVIEW SCHEDULING MODAL (Super Admin) ── --}}
    <div id="interviewModalSA" class="hidden fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 print:hidden">
        <div class="relative w-full max-w-xl bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-[#21255E]">
                ...
                <button type="button" onclick="closeInterviewModalSA()" class="text-gray-300 hover:text-white transition-colors p-1.5 rounded-lg hover:bg-white/10 cursor-pointer">
                    <span class="material-symbols-outlined text-[20px]">close</span>
                </button>
            </div>

            {{-- Modal Form --}}
            <form action="{{ route('superadmin.applications.update-status', $application) }}" method="POST" class="p-6 space-y-4">
                @csrf
                @method('PATCH')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="scheduled_date" required min="{{ date('Y-m-d') }}"
                            value="{{ $latestInterviewSA?->scheduled_date ? \Carbon\Carbon::parse($latestInterviewSA->scheduled_date)->format('Y-m-d') : date('Y-m-d', strtotime('+1 day')) }}"
                    ...
                    <div>
                        ...
                        <input type="time" name="scheduled_time" required
                            value="{{ $latestInterviewSA?->scheduled_time ? date('H:i', strtotime($latestInterviewSA->scheduled_time)) : '10:30' }}"
                            class="w-full text-xs font-semibold px-3 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-[#21255E] focus:border-[#21255E] outline-none bg-white text-gray-900 shadow-xs">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="location_type" id="locationTypeInputSA" onchange="toggleLocationPlaceholderSA(this.value)" required
                            class="w-full text-xs font-semibold px-3 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-[#21255E] focus:border-[#21255E] outline-none bg-white text-gray-900 shadow-xs">
                            <option value="in_person" {{ ($latestInterviewSA?->location_type ?? 'in_person') === 'in_person' ? 'selected' : '' }}>
                                In-Person (Campus / Office)
                            </option>
                            <option value="online" {{ ($latestInterviewSA?->location_type ?? '') === 'online' ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" name="location_address_or_link" id="locationAddressInputSA" required
                            placeholder="e.g. Pared Campus"
                            value="{{ $latestInterviewSA?->location_address_or_link ?? 'Pared Campus' }}"
                            class="w-full text-xs font-semibold px-3 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-[#21255E] focus:border-[#21255E] outline-none bg-white text-gray-900 shadow-xs">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="panel_members"
                        ...
                        value="{{ $latestInterviewSA?->panel_members ?? '' }}"
                ...
                <div>
                    ...
                    <textarea name="remarks" rows="2"
                        ...
                        class="w-full text-xs font-medium p-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-[#21255E] focus:border-[#21255E] outline-none bg-white text-gray-900 shadow-xs">{{ $latestInterviewSA?->remarks ?? '' }}</textarea>
                </div>
                ...
                <div class="flex items-center justify-end gap-2.5 pt-3 border-t border-gray-100">
                    <button type="button" onclick="closeInterviewModalSA()"
    ...
    <script>
        function openInterviewModalSA() {
            const modal = document.getElementById('interviewModalSA');
            if (modal) {
        ...
        function closeInterviewModalSA() {
            const modal = document.getElementById('interviewModalSA');
            if (modal) {
                modal.classList.add('hidden');
                document.body.style.overflow = '';
            }
            const select = document.getElementById('stageSelectSA');
            if (select) {
                select.value = '{{ $application->status }}';
            }
        }

        function selectStageSA(st) {
            if (st === 'interview_scheduled') {
                openInterviewModalSA();
            } else {
                const input = document.getElementById('stageStatusInputSA');
                const form = document.getElementById('stageFormSA');
                if (input && form) {
                    input.value = st;
                    form.submit();
                }
        ...
        function handleStageChangeSA(select) {
        ...
        function toggleLocationPlaceholderSA(type) {
            const input = document.getElementById('locationAddressInputSA');
            if (!input) return;
            if (type === 'online') {
                ...
                input.placeholder = 'e.g. Main Campus - Administrative Boardroom (Block B)';                if (!input.value || input.value.includes('meet.google.com')) {
