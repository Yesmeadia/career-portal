...
@section('title', 'Manage All Job Vacancies')
...
@section('content')
    <div class="max-w-[1400px] mx-auto">
            ...
            <div>
                <div class="flex items-center gap-3">
                    <h2 class="text-4xl font-extrabold text-[#111827] tracking-tight leading-none" style="font-size: 38px;">
                        Manage All Job Vacancies</h2>
                </div>
            </div>

            {{-- Top Right Actions --}}
            <div class="flex items-center gap-3">
                <a href="{{ route('superadmin.vacancies.create') }}"
        ...
        <div class="bg-white shadow-sm border border-gray-100 p-4 mb-6" style="border-radius: 20px;">
            <form action="{{ route('superadmin.vacancies.index') }}" method="GET"
                class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="flex flex-1 items-center gap-3 w-full flex-wrap sm:flex-nowrap">
                    <div class="relative flex-1 min-w-[200px]">
                        ...
                        <input type="text" name="search" value="{{ $filters['search'] ?? '' }}"
                            placeholder="Search by title, department, or school..."
                    ...
                    <select name="school_id"
                        ...
                        @foreach($schools as $sch)
                            <option value="{{ $sch->id }}" {{ isset($filters['school_id']) && (int) $filters['school_id'] === $sch->id ? 'selected' : '' }}>
                                {{ $sch->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <table class="w-full text-left text-sm border-collapse">
                <thead>
                    <tr
                        ...
                        <th class="px-6 py-4">School / Institution</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($vacancies as $job)
                        <tr class="hover:bg-gray-50/60 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 0  font-bold text-xs rounded-full inline-block">
                                    {{ $job->school->name ?? 'YES India Foundation' }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <a href="{{ route('superadmin.applications.index', ['vacancy_id' => $job->id]) }}"
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form action="{{ route('superadmin.vacancies.toggle-status', $job) }}" method="POST" class="inline">
                                        @csrf @method('PATCH')
                                        <button type="submit"
                                    ...
                                    <a href="{{ route('superadmin.vacancies.edit', $job) }}"
                                    ...
                                    <form action="{{ route('superadmin.vacancies.destroy', $job) }}" method="POST"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-400">
                                No vacancies found. Post a new vacancy above.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
