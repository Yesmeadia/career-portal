...
@section('title', 'Post New Vacancy — Super Admin')
@section('page-title', 'Create Vacancy')
@section('page-subtitle', 'Fill out the form below to post a new job vacancy for any school institution')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <form action="{{ route('superadmin.vacancies.store') }}" method="POST" class="space-y-8">
        @csrf
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm">
            ...
            <div>
                <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Select School <span class="text-red-500">*</span></label>
                <select name="school_id" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm bg-white font-figtree font-semibold">
                    ...
                    @foreach($schools as $sch)
                        <option value="{{ $sch->id }}" {{ old('school_id') == $sch->id ? 'selected' : '' }}>
                            {{ $sch->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="vacancy_type" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm bg-white font-figtree">
                        <option value="campus" {{ old('vacancy_type') === 'campus' ? 'selected' : '' }}>Campus Wide</option>
                        <option value="class" {{ old('vacancy_type') === 'class' ? 'selected' : '' }}>Specific Class Level</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="global_class_id" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm bg-white font-figtree">
                        ...
                        <option value="{{ $gc->id }}" {{ old('global_class_id') == $gc->id ? 'selected' : '' }}>{{ $gc->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="location" value="{{ old('location') }}" placeholder="e.g. South Campus, New Delhi"
                           class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm font-figtree">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="min_qualification" value="{{ old('min_qualification') }}" placeholder="e.g. Master's in English / B.Ed."
                           class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm font-figtree">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="salary_from" value="{{ old('salary_from') }}" placeholder="e.g. 25000"
                           class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm font-figtree">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Salary Range To (₹/month)</label>
                    <input type="number" name="salary_to" value="{{ old('salary_to') }}" placeholder="e.g. 40000"
                           class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm font-figtree">
                </div>
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Initial Status <span class="text-red-500">*</span></label>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="5" required placeholder="Detailed responsibilities and candidate expectations..."
