    ...
    <div class="max-w-[1400px] mx-auto">
        ...
        <div class="bg-white shadow-sm border border-gray-100 overflow-hidden mb-6" style="border-radius: 20px;">
            ...
            <div class="p-6 border-b border-gray-100 space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div class="flex items-center gap-3">
                        <div class="flex items-center justify-center text-blue-600 shrink-0"
                            style="width:36px;height:36px;border-radius:12px;background:#eff6ff;">
                            <span class="material-symbols-outlined" style="font-size:20px;">groups</span>
                        </div>
                        <div>
                            <h3 class="text-lg font-bold text-[#111827]">Applications Directory</h3>
                            <p class="text-xs text-gray-500">Review candidate profile submissions, manage application
                                stages, and evaluate profiles.</p>
                        </div>
                    </div>
                ...
                {{-- Filter Toolbar --}}
                <form method="GET" action="{{ route('schooladmin.applications.index') }}">
                    <div class="flex flex-wrap gap-3 items-center">
                        ...
                        <div class="relative flex-1 min-w-[220px] max-w-xs">
                            ...
                            <input type="text" name="search" value="{{ $filters['search'] ?? '' }}"
                                ...
                                class="w-full bg-white border border-gray-200 text-[13px] text-gray-700 placeholder-gray-400 pl-10 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#21255e]/10 transition-all"
                        ...
                        <div class="relative min-w-[160px]">
                            ...
                            <select name="status"
                                class="w-full bg-white border border-gray-200 text-[13px] text-gray-700 pl-9 pr-8 py-2.5 appearance-none focus:outline-none focus:ring-2 focus:ring-[#21255e]/10 transition-all cursor-pointer"
                                style="border-radius: 12px;">
                                <option value="">All Statuses</option>
                                @foreach(['submitted', 'new', 'under_review', 'shortlisted', 'interview_scheduled', 'interview_completed', 'selected', 'rejected', 'on_hold', 'hired'] as $st)
                        ...
                        <div class="relative min-w-[180px]">
                            <span
                                class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none"
                                style="font-size:16px;">work</span>
                            <select name="vacancy_id"
                                class="w-full bg-white border border-gray-200 text-[13px] text-gray-700 pl-9 pr-8 py-2.5 appearance-none focus:outline-none focus:ring-2 focus:ring-[#21255e]/10 transition-all cursor-pointer"
                                style="border-radius: 12px;">
                                <option value="">All Vacancies</option>
                                @foreach($vacancies as $vac)
                                    <option value="{{ $vac->id }}" {{ ($filters['vacancy_id'] ?? '') == $vac->id ? 'selected' : '' }}>
                                        {{ $vac->title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <button type="submit"
                            class="flex items-center gap-2 text-white text-[13px] font-bold px-5 py-2.5 transition-all hover:opacity-90 active:scale-95 shadow-sm cursor-pointer"
                            ...
                            <tr style="background: {{ $rowBg }}; transition: background 0.15s;"
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="min-w-0">
                                            <div class="flex items-center gap-1.5">
                                                <a href="{{ route('schooladmin.applications.show', $app) }}"
                                                    class="font-bold text-[#111827] hover:text-[#21255E] transition-colors block truncate"
                                                    style="font-size:13px;max-width:170px;">{{ $app->full_name }}</a>
                                            </div>
                                            <span
                                                class="text-[11px] text-gray-400 font-mono">{{ $app->reference_no ?? $app->application_number }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-5 py-3.5 border-b border-gray-50">
                                    <p class="font-semibold text-[#111827]" style="font-size:13px;">
                                        {{ Str::limit($app->vacancy?->title ?? 'General Application', 30) }}
                                    </p>
                                    <p class="text-[11px] text-gray-400 mt-0.5">
                                        {{ $app->vacancy?->department->name ?? 'General Dept' }}
                                    </p>
                                </td>
                                ...
                                <td class="px-5 py-3.5 border-b border-gray-50">
                                    ...
                                    <p class="text-[11px] text-gray-400 mt-0.5">
                                        ...
                                        @else
                                            <span class="text-blue-500 font-medium">Fresh Candidate</span>
                                        @endif
                                    </p>
                                </td>
                            ...
                            <tr>
                                <td colspan="6" class="py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <a href="{{ route('schooladmin.applications.index') }}"
                                            class="text-[12px] font-bold text-[#21255E] hover:underline mt-1">Clear all
                                            filters</a>
                                    </div>
                                </td>
                            </tr>
